...
自动选择会读取 IPv4 默认路由接口。只有一个候选时直接使用；存在多个候选时，程序分别执行不带凭据的联网及 NJUPT Portal 状态探测，优先选择当前需要认证的 NJUPT 接口。多个接口均被确认属于 NJUPT 时按默认路由 metric 选择；只有普通联网证据且无法区分时拒绝猜测，可用 `--interface ens33` 手动指定。
...
界面支持输入校园网账号和密码、选择运营商和网卡、保存登录信息、安装并启用开机自启服务、查看服务状态以及卸载服务。接口默认为 `auto`；密码输入框会遮蔽内容，已保存凭据存在时留空表示沿用原密码。
