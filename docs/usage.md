...
sudo apt install ./dist/njupt-autologin_0.8.0_all.deb
```
...
Tk 界面只调用统一的平台服务适配器。当前 Debian 版本选择 Linux 适配器，由它封装 `iproute2`、systemd 和 linger；后续 Windows 版本可提供同一接口的 Windows 适配器，无需改动窗口逻辑。平台包在构建时只复制公共适配器定义和目标平台实现，Debian 包不会携带其他平台的适配器源码。
